...
一个面向私有、高质量论文集合的可复现、证据可追溯研究 Agent 工程。

项目重点不是让模型自由拼接论文结论，而是把论文发现、双语混合检索、最小事实编译、
引用验证、跨轮状态恢复和高风险工具审批收敛到可测试的确定性边界。当前已经形成从冻结语料、
图文检索和记忆，到主 Agent 自适应编排、受控执行子图、事实级证据台账、可恢复事件流与
私有研究界面的本地闭环。
...
- **事实级证据编译**：多论文比较由代码确定性解析目标与维度，模型只提出受契约约束的最小事实需求；每个“论文 × 维度”网格单元独立检索、编译和提交，失败单元可精确重试，最终答案再由已验证事实确定性渲染，不让答案模型重新改写证据关系。
...
在代码版本 `0d32441` 上，固定 Top 4 的五题 v2 门禁取得 5/5 成功；15/15 条强制事实全部完成
检索、水化、Compiler 可见、编译、确定性答案输入、表达和正确引用链路，30/30 个请求单元通过，
最终失败单元为 0。五题真实主 Agent 运行中，两题命中确定性快路，三题因复杂或模糊而保守进入
完整规划；快路的主路由加任务物化 p50 约 15 ms，加上下文水化后的主编排前奏 p50 约 63 ms。

这是一组固定五题开发门禁，不能外推为全量生产分布结论；v2 金标也仍待人工双审与仲裁。
较早的 30 题全量快照位于代码版本 `66810d5`，取得 27/30 成功、0 超时和 99.1% 的已表达事实
引用正确率。最新自适应编排、事实链路、模型裁判方差和回滚边界见
[主 Agent 自适应编排与比较规划维度修复复测](reports/主Agent自适应编排与比较规划维度修复复测-v1.md)，
旧全量口径见[多论文比较最小证据编译器 v2 评测摘要](reports/多论文比较最小证据编译器-v2-评测摘要.md)。

## 主 Agent 统一编排

生产启动首先进入跨轮次主 Agent 主体图（`paper_research_main_v1`），不是先
进入某个执行子图。主图先恢复最近历史、滚动摘要、活动目标、会话任务计划、
远距会话召回与长期记忆，再解释本轮变化、对齐目标和规划任务；长期记忆最多
召回 5 条并受 5,000 字符预算约束，读取失败时降级为空而不阻断请求。只有完成这些
...
系统里的“记忆/上下文”不是四套同义数据库：最近消息窗口用于保持当前对话连贯，
远距会话召回用于找回较早轮次，下面的短期 RAG 记忆用于 24 小时内的追问改写，
长期记忆则保存经明确意图和审批确认的偏好、项目背景或已确认结论。Conversation Store
中的滚动摘要、目标和任务计划属于主 Agent 工作区状态，也会参与水合，但不是长期记忆。
...
“自动使用本地论文知识库”表示由来源意图选择本地论文，并可与动态研究组合；“仅依据本地论文回答”
则禁止外部研究。显式模式与“使用/不要使用知识库、联网”等语言要求冲突时，请求在执行前返回
可操作的 `422`，不会静默覆盖。检查器显示最近窗口、远距候选、采用 turn ID/相关度、能力计划和实际检索路径。
...
执行路径；本地 RAG 将其装配为独立的低信任 JSON 层，记忆 ID 永远不会进入 citation（引用）
集合，`confirmed_conclusion` 仍须由本轮论文证据重新核验。水合观测只记录各阶段耗时、数量、
最终字符数和保守 Token 估算，不记录问题、记忆正文、记忆 ID 或 Provider 原始载荷。
...
组装顺序固定为可信系统规则、必要对话历史、低信任短期记忆、低信任长期记忆、当前用户问题与
任务状态、检索证据。历史、记忆、任务状态和论文正文均属于低信任数据；记忆与证据分别使用
...
一个 Uvicorn worker（工作进程），让所有请求共享索引和模型；同一会话的问答保持串行，
不同会话默认最多并发两个完整问答，容量由 `PRA_WEB_MAX_INFLIGHT_RUNS`（范围 1–16）控制。
容量耗尽时新请求快速返回繁忙响应，不建立无界队列；设为 `1` 可回退为全局串行。
论文比较的单次检索阶段最多并发六路。服务只监听 `127.0.0.1:8092`，不依赖 Nginx 或公网域名。
